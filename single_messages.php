...
if (!empty(get_first_line_db($db, "SELECT * FROM friend_requests where ( from_id = '" . $_SESSION["user_id"] . "' OR to_id = '" . $msg_id . "') and accepted = 1"))) {
	$result = get_all_lines_db($db, "SELECT messages.sender_id , messages.id ,content,attachements,msg_read,friendship_id,f.from_id,f.to_id , messages.time , f.from_id,f.to_id FROM messages inner join friend_requests as f on f.id = messages.friendship_id	WHERE friendship_id = '" . $conv_id . "' order by messages.time DESC");
	$e = [];
